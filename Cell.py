...
class Cell:
    ...
    def load_sat(self, filepath, i):
        ...
        for row in csv_reader: # there's only one row, but this extracts it
            ...
            self.tau_do[i], self.alpha_S[i], self.alpha_D[i] = float(row[3]), float(row[4]), float(row[5])
            self.alpha_N[i], self.alpha_R[i], self.P[i] = float(row[6]), float(row[7]), float(row[8])
            self.AM_sat[i], self.tau_sat[i], self.C_sat[i] = float(row[9]), float(row[10]), float(row[11])
            self.expl_rate_L[i], self.expl_rate_D[i] = float(row[12]), float(row[13])
        csv_file.close()
